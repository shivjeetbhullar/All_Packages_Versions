...
def render_comphis_url(headers,SYS,fdate,tdate,series,dtype):
    symbol = f"https://www1.nseindia.com/marketinfo/sym_map/symbolCount.jsp?symbol={SYS}"
    symb = requests.get(symbol,headers=headers).text.strip()
    print(symb)
    return f"https://www1.nseindia.com/products/dynaContent/common/productsSymbolMapping.jsp?symbol={SYS}&segmentLink=3&symbolCount={symb}&series={series}&dateRange=+&fromDate={fdate}&toDate={tdate}&dataType={dtype.upper()}"
